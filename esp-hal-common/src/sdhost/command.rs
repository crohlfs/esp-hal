...
impl Command {
    ...
    pub fn argument(self) -> u32 {
        match self {
            ...
            Self::SendCSD(rca) | Self::AppCommand(rca) | Self::RequestStatus(rca) => {
                (rca as u32) << 16
            }
    ...
    //         Self::RequestStatus(_) => mem::size_of::<response::R2>(),
...
mod test {
    ...
    fn test_command_to_bytes() {
        ...
        let cmd = Command::RequestStatus(0);
        let bytes: [u8; 6] = cmd.into();
        assert_eq!(bytes, hex!("49 00 00 00 00 AF"));
    }
}
